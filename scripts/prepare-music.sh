...
for key in "${tracks[@]}"; do
  ...
  # The original owner-supplied masters are Opus-in-Ogg. Minecraft 1.20.1 does
  # not decode those reliably, so CI converts the complete stream to Ogg Vorbis.
  # No -t/-ss filter is used: every packet in the source is preserved.
