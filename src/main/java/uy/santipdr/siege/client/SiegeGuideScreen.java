...
/**
 * SIEGE 2.0 reference browser. Knowledge and equipment are deliberately split:
 * general world/reference material lives in ARCHIVE, while ITEMS are exposed only
 * through ARMORY. Unit dossiers remain owned by IntelScreenV3.
 */
public final class SiegeGuideScreen extends Screen {
    public enum Mode { ARCHIVE, ARMORY }

    private final Screen parent;
    private final Mode mode;
    private SiegeGuideData.Category category;
    ...
    private SiegeButton prevPage, nextPage, up, down, pictures, reel, reveal;
    ...
    public SiegeGuideScreen(Screen parent) { this(parent, Mode.ARCHIVE); }

    public SiegeGuideScreen(Screen parent, Mode mode) {
        super(Component.literal("SIEGE"));
        this.parent = parent;
        this.mode = mode == null ? Mode.ARCHIVE : mode;
        this.category = categories().get(0);
    }

    private List<SiegeGuideData.Category> categories() {
        if (mode == Mode.ARMORY) return List.of(SiegeGuideData.Category.ITEMS);
        return List.of(SiegeGuideData.Category.LORE, SiegeGuideData.Category.OPERATIONS,
                SiegeGuideData.Category.DIFFICULTY, SiegeGuideData.Category.CHRONICLES,
                SiegeGuideData.Category.INSPIRATIONS);
    }
    ...
    @Override protected void init() {
        ...
        if (mode == Mode.ARCHIVE && width >= 330) {
            SiegeButton manual = addRenderableWidget(new SiegeButton(width - 118, 7, 110, 19,
                    text("MANUAL DE CAMPO", "FIELD MANUAL"), b -> {
                remember(); SiegeUiSounds.confirm(); minecraft.setScreen(new SiegeArchiveScreen(this));
            }, SiegeTheme.GOLD).withIcon("overview").setCompactCenter(true));
            manual.setTooltip(Tooltip.create(text(
                    "Estados de trauma, misiones y protocolos operativos.",
                    "Trauma states, missions and operational protocols.")));
        }

        int i = 0;
        List<SiegeGuideData.Category> available = categories();
        for (var value : available) {
            ...
            SiegeButton tab = new SiegeButton(8 + slot % layout.columns() * (layout.tabWidth() + 4),
                ...
                search.setValue(""); refreshEntries(); SiegeUiSounds.category();
        ...
        search = new EditBox(font, 8, layout.searchY(), width - 48, 18,
                mode == Mode.ARMORY ? text("Buscar equipo", "Search equipment") : text("Buscar en el archivo", "Search archive"));
        search.setHint(mode == Mode.ARMORY ? text("Buscar objeto o equipo…", "Search item or equipment…")
                : text("Buscar en esta sección…", "Search this section…"));
        ...
        int mediaX = a.x() + 56;
        int availableMedia = Math.max(1, a.w() - 56);
        int imageW = Math.min(82, Math.max(48, availableMedia / 2 - 2));
        pictures = addRenderableWidget(new SiegeButton(mediaX, layout.footerY(), imageW, 18,
                text("IMÁGENES", "IMAGES"), b -> openImage(0), SiegeTheme.GOLD).setCompactCenter(true));
        pictures.setTooltip(Tooltip.create(text("Abrir evidencia visual a tamaño completo", "Open visual evidence at full size")));

        int reelX = mediaX + imageW + 4;
        int reelW = Math.max(1, Math.min(74, a.x() + a.w() - reelX));
        reel = addRenderableWidget(new SiegeButton(reelX, layout.footerY(), reelW, 18,
                text("REEL", "REEL"), b -> openReel(), SiegeTheme.CYAN).withIcon("image").setCompactCenter(true));
        reel.setTooltip(Tooltip.create(text(
                "Reproducir la secuencia de evidencia recuperada del video.",
                "Play the evidence sequence recovered from the supplied video.")));
    ...
    private void refreshEntries() {
        ...
        List<SiegeGuideData.Category> available = categories();
        int index = 0;
        for (SiegeButton button : tabButtons) button.setSelected(available.get(index++) == category);
    ...
    private void buildList() {
        ...
        for (int i = page * layout.capacity(); i < Math.min(entries.size(), (page + 1) * layout.capacity()); i++) {
            ...
            }, mode == Mode.ARMORY ? SiegeTheme.GOLD : SiegeTheme.RED).setSelected(item == entry);
    ...
    private void buildArticle() {
        ...
        pictures.visible = entry != null && !entry.images().isEmpty();
        reel.visible = isReelEntry() && layout.article().w() >= 170;
    ...
    private boolean isReelEntry() {
        return entry != null && entry.id().equals("third-justice") && entry.images().size() >= 3;
    }
    ...
    private void openImage(int index) {
        ...
        remember(); SiegeUiSounds.dossier(); minecraft.setScreen(new SiegeGuideImageScreen(this, entry.images(), index));
    }

    private void openReel() {
        if (!isReelEntry() || locked()) return;
        remember(); SiegeUiSounds.confirm(); minecraft.setScreen(new SiegeEvidenceReelScreen(this, entry.images()));
    }
    ...
    @Override public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        String title = mode == Mode.ARMORY ? label("ARSENAL SIEGE", "SIEGE ARMORY") : label("ARCHIVO SIEGE", "SIEGE ARCHIVE");
        g.drawCenteredString(font, title, (80 + width - 8) / 2, 12, SiegeTheme.INK);
        var a = layout.article();
        SiegeTheme.panel(g, a.x(), a.y(), a.w(), a.h(), mode == Mode.ARMORY ? SiegeTheme.GOLD : SiegeTheme.RED);
