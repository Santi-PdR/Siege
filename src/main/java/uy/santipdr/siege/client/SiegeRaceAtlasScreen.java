...
public final class SiegeRaceAtlasScreen extends Screen {
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        g.drawString(font, fit(label(
                "Elegí una raza para ver rareza, forma de progresión y lo importante para entenderla.",
                "Choose a race to see rarity, progression style and the important things to know."), panelW - 24),
    ...
    private void renderRarityStrip(GuiGraphics g) {
        ...
        for (int i = 0; i < order.size(); i++) {
            ...
            String suffix = i + 1 < order.size() ? "  >" : "";
            int itemWidth = font.width(rarity.label(spanish()) + suffix) + 5;
            ...
            if (cursor + itemWidth > right) {
                // This should only happen on extremely narrow windows; fit the final label
                // instead of silently dropping a rarity tier.
                String fitted = fit(rarity.label(spanish()), Math.max(8, right - cursor));
                g.drawString(font, fitted, cursor, y, rarity.accent(), false);
                continue;
            }
            g.drawString(font, rarity.label(spanish()), cursor, y, rarity.accent(), false);
            cursor += font.width(rarity.label(spanish()));
            if (!suffix.isEmpty()) {
    ...
    private void renderDetail(GuiGraphics g, int mouseX, int mouseY) {
        ...
        SiegeKnowledgeData.Entry knowledge = selected.knowledgeId().isBlank() ? null : SiegeKnowledgeData.get(selected.knowledgeId());
        String body = knowledge != null ? knowledge.body(spanish()) : selected.summary(spanish());
        ...
        int buttonY = detailY + detailH - 24;
        int maxY = buttonY - 5;
        g.enableScissor(detailX, textTop - 1, detailX + detailW, maxY);
        int yy = textTop;
        for (int i = first; i < lines.size() && yy + font.lineHeight <= maxY; i++) {
            g.drawString(font, lines.get(i), x, yy, i == first ? SiegeTheme.INK : SiegeTheme.INK, false);
            yy += font.lineHeight + 2;
        }
        g.disableScissor();

        if (!selected.knowledgeId().isBlank()) {
            ...
            String open = label("VER EN ENCICLOPEDIA", "OPEN IN ENCYCLOPEDIA");
